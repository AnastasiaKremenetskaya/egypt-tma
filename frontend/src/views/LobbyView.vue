...
const { state, isAdmin, userId, startGame, leaveGame } = useGame()
...
<template>
  <div class="lobby">
    ...
    <!-- code card with copy animation -->
    <div class="code-card" :class="{ copied }" @click="copyCode">
      <span class="code-label">Код комнаты</span>
      <span class="code-value">{{ room.code }}</span>
      <transition name="fade">
        <span v-if="copied" class="code-hint copied-hint">✓ Скопировано!</span>
        <span v-else class="code-hint">нажми чтобы скопировать</span>
      </transition>
    </div>
      ...
      <div class="players-list">
        ...
        >
          ...
          <div class="player-info">
            <span class="player-name">«{{ p.title }}» @{{ p.username }}</span>
          </div>
    ...
    <div class="action-area">
      ...
      <template v-else>
        <p class="wait-hint">Ждём когда организатор начнёт суд…</p>
        <div class="waiting-dots">
          <span v-for="i in 3" :key="i" class="dot" :style="{ animationDelay: (i * .2) + 's' }" />
        </div>
      </template>

      <button class="btn-leave" @click="onLeave">🚪 Выйти из Храма</button>
    </div>
  </div>
</template>
...
.code-card.copied { background: rgba(76,175,80,.12); border-color: rgba(76,175,80,.45); }
...
.code-hint { display: block; font-size: 10px; color: #8a9bb5; margin-top: 6px; opacity: .7; }
.copied-hint { color: #4caf50 !important; opacity: 1 !important; font-weight: bold; }
...
.invite-link { color: #e8b84b; text-decoration: none; font-size: 14px; font-weight: bold; }
...
.player-avatar {
  ...
  background: rgba(201,146,42,.2); border: 1px solid rgba(201,146,42,.35);
  color: #e8b84b; font-size: 15px; font-weight: bold;
  display: flex; align-items: center; justify-content: center; flex-shrink: 0;
}
.player-info { flex: 1; min-width: 0; }
.player-name { display: block; font-size: 14px; color: #fef9e7; }
.admin-badge {
  font-size: 10px; color: #c9922a;
  border: 1px solid rgba(201,146,42,.4); border-radius: 6px;
  padding: 2px 7px; white-space: nowrap;
}

.action-area { margin-top: 4px; display: flex; flex-direction: column; gap: 10px; }
.btn-gold {
  width: 100%; padding: 15px;
  background: linear-gradient(135deg, #c9922a, #e8b84b);
  color: #04060f; border: none; border-radius: 12px;
...
.btn-leave {
  ...
  border: 1px solid rgba(232,75,75,.35);
  border-radius: 12px; color: rgba(232,75,75,.8);
  font-family: inherit; font-size: 14px; cursor: pointer;
  transition: background .2s;
}
.btn-leave:hover { background: rgba(232,75,75,.08); }

.wait-hint { text-align: center; color: #8a9bb5; font-size: 13px; font-style: italic; }
